...
\section{Prévisions}
\label{sec:prev}
Pour répondre à la question du directeur, un individu comportant les caractéristiques suivantes,
CLMAGE=45, SEATBELT=1, ATTORNEY=1, MARITAL="single" et  CLMINSUR = 1 aura selon le modèle actuelle une perte économique de : $Y \in [0.3378229, 46.89545]$\\ à un niveau de 95\%.
\\
\\
