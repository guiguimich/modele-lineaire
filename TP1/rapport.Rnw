...
Suite à la selection des variables, il nous a été possible de déterminer qu'il n'y avait qu'une seule
...
De plus, voici le $R^2_a$ : $0.2754$. Ainsi, une grande variabilité de la variable endogène n'est pas expliqué par le modèle. Ceci est en parti dû au grand nombre de variables qualitives dans le modèle qui n'ont pas assez de valeurs possibles afin de réfléter l'étendue des valeurs possible de la perte économique. \\
...
<<echo = FALSE, fig.cap="\\label{fig:rstudent}Résidus studentisé en fonction de CLMAGE",fig.pos='htb'>>=
plot(data$CLMAGE,rstudent(modele),xlab="CLMAGE", ylab="Résidus studentisés")
@

\newpage
\subsection{Homogénéité}
<<echo = FALSE,fig.cap="\\label{fig:homo}Résidus studentisé en fonction de Y",fig.pos='htb'>>=
...
\\
\\
Avec les même caractéristiques, on obtient : $E[Y] \in [3.369781, 4.701301]$. Ainsi, on constate la grande variabilité de la perte économique en comparant les différentes bornes des deux prévisions.

\begin{table}[htb]
\centering
\addtolength{\leftskip} {-2cm}
\addtolength{\rightskip}{-2cm}
\caption{Intervalles de confiances des moyennes de la perte économique totale prévues}
...
\item LOSS\\
% À débattre.
Cette colonne contient une valeur très extrême de \textit{1067.697}. En analysant la figure \ref{fig:boxplot}, on voit bien que la valeur est très énorme à comparer au reste. Néanmoins, comme il s'agit de perte économique aux États-Unis suite à une blessure corporelle, cette valeur est possible donc nous ne l'avons pas retiré de l'étude.\\

<<echo = FALSE,fig.cap="\\label{fig:boxplot}Boxplot de LOSS",fig.pos = "htb">>=
...
\newpage
\item CLMAGE\\
Cette colonne contient une valeur de \textit{610}, valeur impossible selon la description de la variable, soit l'âge du réclamant. Afin de ne pas fausser les résultat, cette valeur a été modifié pour 61.
\end{enumerate}
\end{enumerate}
