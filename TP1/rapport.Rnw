...
\label{ssec:var_qual}
Les variables qualitatives du modèle, soit \texit{ATTORNEY, SEATBELT et MARITAL} ont chacune été converti en \textit{factor} car c'est le type de données usuelle de R afin d'effectuer des régression linéaire comportant des variables qualitatives.
